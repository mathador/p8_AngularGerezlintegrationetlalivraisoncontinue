...
Exécuter le conteneur (porte 8080):
```bash
docker run --rm -p 8282:80 olympic-app
```

Quelques problématiques rencontrées:
- mauvais chemin dans Dockerfile: 
/app/dist/olympic-games-starter
modifé aec /app/dist/olympic-games-starter/browser
 
### 2. Docker Compose

Un fichier `docker-compose.yml` simple est fourni pour builder et exécuter l'application localement.
